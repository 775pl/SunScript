...
(async () => {
  ...
  try {
    ...
    await page.locator('.contact-form button[type=submit]').evaluate(el => el.disabled = false);
    ...
    assert.equal(await page.locator('.contact-fields').isVisible(), false);
    assert.equal(await page.locator('.sent-sun').evaluate(el => getComputedStyle(el).animationName), 'none');
    await page.locator('.contact-success').screenshot({ path: 'tests/contact-success-mobile.png' });
    assert.equal(await page.locator('#contact-need').inputValue(), '');
    await page.locator('.sent-again').click();
    assert.equal(await page.locator('#contact-name').isVisible(), true);
